...
def clean_dataframe(df_raw: pd.DataFrame, target_column: str) -> pd.DataFrame:
    ...
    # 2. Filter out "Non-Musical" content
    # (Speechiness > 0.66 usually indicates spoken word)
    # This helps the model focus on actual songs as identified in Group 8 EDA.
    if "speechiness" not in df_clean.columns:
        raise KeyError(
            "Expected column 'speechiness' not found in input dataframe.")
    df_clean = df_clean[
    ...
    if "duration_ms" not in df_clean.columns:
        raise KeyError(
            "Expected column 'duration_ms' not found in input dataframe.")
